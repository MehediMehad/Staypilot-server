...
async function run() {
  try {
    ...
    app.get("/category", async (req, res) => {
      try {
        const categories = await roomsCollection
          .aggregate([
            {
              $group: {
                _id: "$category", 
          ...
          .toArray();
        // Map the results to get an array of unique category names
        const uniqueCategories = categories.map((item) => item._id);
...
app.get("/", (req, res) => {
  res.send("Hello from staypilot Server..");
});

app.listen(port, () => {
  console.log(`staypilot is running on port ${port}`);
});
